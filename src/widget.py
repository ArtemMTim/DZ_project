...
from masks import credit_card_masking, bank_account_masking
from datetime import datetime


def masking_with_info(number: str) -> str:
    """Функция принимает на вход сообщение вида: '<Вид банковской карты/счет> <номер>' и
    выводит замаскированный счёт карты/банковского счёта в соответсвии с шаблоном: '<Вид банковской карты/счет> <*****>'"""
    # Разбиваем вводные данные на вид банковской карты/счет и номер
    number_info = number.split()
    # Проверяем введён ли вид банковской карты/счет. В случае отсутствия вида банковской карты/счета выводим сообщение об ошибке в введённых данных.
    if len(number_info) == 1:
        result_message = "Ошибка в введённых данных!"
    else:
        # Проверяем, какой номер был введён: карты либо банковского счёта. Выбираем необходимую функцию из модуля "masks".
